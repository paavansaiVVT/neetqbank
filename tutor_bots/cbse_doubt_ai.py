...
from tutor_bots.prompt import cbse_common_prompt
from tutor_bots.cbse_chapter_bot import llm,tools
...
prompt = ChatPromptTemplate.from_messages([("system", cbse_common_prompt),MessagesPlaceholder(variable_name="messages"),MessagesPlaceholder(variable_name="agent_scratchpad")])
agent = create_tool_calling_agent(llm,tools, prompt)
agent_executor = AgentExecutor(agent=agent, tools=tools, verbose=True)

# Generate a response from the model
async def generate_response_cbse_common(request_data:RequestData):
    try:
