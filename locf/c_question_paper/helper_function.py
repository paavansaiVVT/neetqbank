...
import re, json
from locf.c_question_paper.classes import QuestionPaperRequest, ImageTag, required_items, question_type_dict
from locf.c_question_paper import prompts, parser_helper
import os
...
logging.info(f"Mistral client initialized successfully")
...
class ImageTagger:
    ...
    def get_page_batches(self, pdf_url: str, batch_size=5):
        ...
        try:
            ...
            logging.info(f"PDF has {total_pages} pages, split into {len(batches)} batches")
            return batches
    ...
    def perform_basic_ocr(self, file_url, batch):
        ...
        try:
            ...
            logging.info(f"Successfully performed basic OCR for batch {batch}")
            return pdf_response
        except Exception as e:
            logging.error(f"Error in Mistral OCR on batch {batch}: {e}")
            return None
    ...
    async def generate_image_tags(self, pdf_response):
        ...
        logging.info(f"Sending {len(image_ids)} image IDs for tagging")
        try:
            ...
            logging.info(f"Received {len(tags_as_dicts)} tags")
...
class llm_function:
    ...
    def get_page_batches(self, pdf_url: str) -> Optional[List[List[int]]]:
        ...
        try:
            ...
            logging.error(f"Error reading PDF from URL: {e}")
            return None
    ...
    def mistral_ocr(self, file_url: str, pages: List[int]):
        ...
        try:
            ...
            logging.error(f"OCR error on pages {pages}: {e}")
            return None
    ...
    async def llm_ocr_process(self, file_url: str):
        ...
        try:
            ...
            for i, batch in enumerate(batches, start=1):
                logging.info(f"Processing batch {i}/{len(batches)}: pages {batch}")
            ...
            logging.error(f"Error in llm_ocr_process: {e}")
            return None
...
class question_paper_function:
    def _validate_questions_extracted(self, parsed_json: dict, expected_count: Optional[int] = None) -> Tuple[bool, List[int], int]:
        ...
        try:
            ...
            if not is_complete:
                logging.warning(f"⚠️ Validation: Expected {expected_count} questions, found {actual_count}. Missing: {missing}")
    ...
    def _merge_question_extractions(self, base_result: dict, retry_result: dict) -> dict:
        ...
        try:
            ...
            logging.error(f"Error merging extractions: {e}")
            return base_result
    
    async def question_paper_extraction(
        self, 
        pdf_url: str, 
        ...
        logging.info(f"🎯 Target: Extract ALL {expected_question_count} questions from PDF ({num_batches} batches)")
        
        # Process questions in batches
        return await self._extract_questions_in_batches(
            pdf_url=pdf_url,
    ...
    async def _extract_questions_in_batches(
        self,
        pdf_url: str,
        ...
        logging.info(f"📦 Batch plan: {len(batches)} batches")
        ...
        async def extract_batch_async(batch_idx, start_q, end_q):
            """Async wrapper for batch extraction"""
            logging.info(f"📦 Processing batch {batch_idx}/{len(batches)}: Questions {start_q}-{end_q}")
            
            # Run synchronous extraction in thread pool
            loop = asyncio.get_event_loop()
            batch_result, batch_tokens = await loop.run_in_executor(
                None,
            ...
            if batch_result and "questions" in batch_result:
                ...
                logging.info(f"✅ Batch {batch_idx} complete: {len(batch_questions)} questions extracted")
                return batch_idx, batch_result, batch_tokens, None
            else:
                error_msg = f"Batch {batch_idx} failed to extract questions {start_q}-{end_q}"
                logging.error(f"❌ {error_msg}")
                return batch_idx, None, batch_tokens, error_msg
        
        # Run all batches in parallel
        tasks = [
            extract_batch_async(batch_idx, start_q, end_q)
        ...
        logging.info(f"⚡ Parallel extraction complete: {len(batches)} batches processed in {end_time - start_time:.2f} seconds")
        
        # Process results
        errors = []
        for response in batch_responses:
            if isinstance(response, Exception):
                logging.error(f"Batch extraction failed with exception: {response}")
                if strict_mode:
                    raise response
                continue
            
            batch_idx, batch_result, batch_tokens, error_msg = response
        ...
        if errors and strict_mode:
            raise ValueError(f"Multiple batch failures: {errors}")
        
        # ✅ CRITICAL: Deduplicate questions by question_number
        # Filter to only include questions in range [1, expected_question_count]
        logging.info(f"🔍 Deduplicating and filtering batch results: {len(all_questions)} raw questions")
        ...
        for q in all_questions:
            ...
            if q_num < 1 or q_num > expected_question_count:
                out_of_range_questions.append(q_num)
                continue
        ...
        if out_of_range_questions:
            logging.warning(f"⚠️ Removed {len(out_of_range_questions)} out-of-range questions: {sorted(set(out_of_range_questions))}")
        
        if duplicate_questions:
            logging.warning(f"⚠️ Removed {len(duplicate_questions)} duplicate questions: {sorted(set(duplicate_questions))}")
        
        logging.info(f"✅ Final unique questions in range: {len(deduplicated_questions)}")
        ...
        logging.info(f"📊 Final results: {len(batches)} batches processed, {actual_count}/{expected_question_count} questions extracted")
                
        if is_complete:
            logging.info(f"✅ SUCCESS: All {expected_question_count} questions extracted!")
            return combined_result, extraction_tokens
        else:
            logging.error(f"❌ BATCH EXTRACTION INCOMPLETE: {actual_count}/{expected_question_count} questions extracted, missing: {missing}")
    ...
    def _extract_single_batch(
        ...
        for attempt in range(max_retries):
            try:
                logging.info(f"📄 Batch {batch_number} - Attempt {attempt + 1}/{max_retries}")
                
                # Build prompt for this batch
                if attempt > 0 and missing_questions:
                    missing_list_str = ', '.join(map(str, missing_questions))
                    logging.info(f"🎯 Focusing on missing questions: {missing_list_str}")
...
"""
                ...
                end_time = time.time()
                logging.debug(f"⏱️ API call: {end_time - start_time:.2f}s")
                ...
                if hasattr(response, 'usage_metadata') and response.usage_metadata:
                    ...
                    logging.debug(f"🎯 Tokens: In={prompt_tokens}, Out={output_tokens}, Total={total_tokens_count}")
                ...
                if not parsed_json:
                    logging.error(f"❌ Attempt {attempt + 1}: Failed to parse response")
                    continue
                ...
                logging.info(f"📊 Batch {batch_number} validation: {actual_count}/{expected_count} questions extracted")
                ...
                if attempt < max_retries - 1:
                    logging.warning(f"🔄 Batch {batch_number} retry {attempt + 2}/{max_retries} for: {missing_questions}")
                else:
                    logging.error(f"⚠️ Batch {batch_number} max retries reached. Missing: {missing_questions}")
                    if strict_mode:
                        raise ValueError(
                ...
                logging.error(f"❌ Batch {batch_number} attempt {attempt + 1} error: {e}")
                if attempt == max_retries - 1:
                    if strict_mode:
    ...
    def _extract_question_count_from_markdown(self, markdown_text: str) -> Optional[int]:
        ...
        for pattern in total_patterns:
            ...
            if match:
                ...
                if 1 <= count <= 500:  # Sanity check
                    logging.info(f"✓ [Strategy 1] Found explicit mention: {count} questions")
                    return count
        ...
        if section_ranges:
            ...
            logging.info(f"✓ [Strategy 2] Found from ranges: {max_from_ranges} questions")
        ...
        if all_question_numbers:
            ...
            if (coverage >= max_qnum * 0.5) or (coverage >= 5 and max_qnum <= coverage * 1.5):
                logging.info(f"✓ [Strategy 3] Extracted from numbering: {max_qnum} questions")
                return max_qnum
            else:
                logging.debug(f"⚠ [Strategy 3] Found numbers but low coverage ({coverage}/{max_qnum}), trying next strategy")
        ...
        if roman_numbers and len(roman_numbers) >= 3:
            ...
            logging.info(f"✓ [Strategy 4] Found roman numerals: {max_roman} questions")
            return max_roman
        ...
        if len(question_markers) >= 5:
            logging.info(f"✓ [Strategy 5] Counted 'Question' markers: ~{len(question_markers)} questions")
        ...
        logging.warning("⚠ Could not reliably extract question count from markdown - all strategies failed")
        return None
    ...
    async def question_preparation(self, gen_response, md_results):
        ...
        """
        try:
            questions_count = qllm_helper_function.get_no_of_questions(gen_response)
            try:
            ...
            try:
                ...
                logging.info(f"✓ [Tier 1] Question count from metadata: {qcount}")
            except (TypeError, ValueError):
                logging.warning("⚠ Metadata extraction failed for question count")
                
                # ===========================================================
                # TIER 2: Use AI agent (most robust, handles any format)
                # ===========================================================
                logging.info("🤖 [Tier 2] Activating Question Count Agent")
                import asyncio
                ...
                if qcount is None or qcount <= 0:
                    ...
                    # ===============================================================
                    logging.info("🔍 [Tier 3] Trying regex pattern matching")
            ...
            logging.error(f"❌ Error in question_preparation: {e}")
            return None, 0
