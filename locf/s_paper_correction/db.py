...
from locf.s_paper_correction import classes
load_dotenv()
            ...
            for item in dict_items:
                result = await session.execute(
                    select(classes.DbAnswerSheetCorrection).where(
                        ...
                        classes.DbAnswerSheetCorrection.question_number == int((item.get("question_number"))),
                    )
                )
...
async def update_exam_details(request: classes.AnswerSheetRequest, student_id: int, exam_details: dict, missed_ans: list, subject_name: str, analysis_data: dict):
    session = get_session()
    try:
        ...
        if existing:
            ...
            existing.max_marks = analysis_data["total_maximum_marks"]
            ...
            new_entry = classes.DbExamDetails(
                ...
                max_marks = analysis_data["total_maximum_marks"],
    ...
    try:
        async with session.begin():
            result = await session.execute(
                ...
                .order_by(classes.QuestionData.question_number)
            )
